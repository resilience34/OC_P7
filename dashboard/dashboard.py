...
# fonction 
def classify_client(model, ID, df, seuil): # fonction via streamlit
    ID = int(ID)
    X = df[df.index == ID]
...
def predict_loan_approval(id_client): # fonction via API
    # Préparer les données pour la requête
    #data = {'id_client': id_client}
    data = dict(id_client = "id_client")

    # Envoyer la requête POST à l'API de prédiction
    ...
    if response.status_code == 200:
        ...
        probability_default_payment = result['probability_default_payment']
        return prediction, probability_default_payment
    else:
